...
const ManageLessons = () => {
  ...
  const toggleFeaturedMutation = useMutation({
    mutationFn: async ({ id, isFeatured }) => {
      return axios.patch(`/admin/lessons/${id}/featured`, {
  ...
  const filteredLessons = allLessons.filter((lesson) => {
    ...
    const matchesPrivacy =
      filterPrivacy === 'All' || lesson.privacy === filterPrivacy;
    const matchesAccess =
  ...
  const handleToggleFeatured = (lesson) => {
    const actionText = lesson.isFeatured
  ...
  const privateLessons = allLessons.filter(
    (l) => l.privacy === 'Private'
  ).length;
  const featuredLessons = allLessons.filter((l) => l.isFeatured).length;
  const premiumLessons = allLessons.filter(
  ...
  return (
    <div className="space-y-8">
      ...
      >
        <h1 className="text-4xl font-bold mb-4 bg-linear-to-r from-green-600 via-blue-600 to-purple-600 bg-clip-text text-transparent">
          Manage Lessons
        </h1>
